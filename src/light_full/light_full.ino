...
void flash(int * pixel, int number, int color[], int bgcolor[], int times){
    for(int i = 0; i < times; i++) {
      set_color(pixel, number, color);
      pixels.show();
      delay(100); 
      set_color(pixel, number, bgcolor);  
      pixels.show();
      delay(100);
    }
}
...
void setup() {
  ...
  if(WiFi.status() == WL_CONNECTED) {
    ...
    flash(ring_index, NUMBER_RING, green, off_color, 2);
    WiFiconnected = true;
  } else {
    flash(ring_index, NUMBER_RING, yellow, off_color, 2);
    WiFiconnected = false;
  }
  startup_animation();
}
...
void loop() {
  ...
  if(WiFiconnected == false) {
    ...
    WiFiconnected = true;
    flash(ring_index, NUMBER_RING, green, red, 2);
  }
  server.handleClient();
